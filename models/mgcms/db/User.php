...
class User extends BaseUser implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['status', 'created_by', 'file_id', 'acceptTerms5', 'acceptTerms6', 'is_company', 'is_verified'], 'integer'],
...
//        [['password'], StrengthValidator::className(), 'min' => 8, 'digit' => 1, 'special' => 1, 'upper' => 1, 'lower' => 1, 'userAttribute' => 'username'],
            ...
            [['facebook', 'twitter', 'linkedin', 'instagram', 'phone', 'position', 'step', 'type', 'is_corespondence', 'house_no', 'companyForSale', 'agent_code', 'description', 'is2faEnabled'], 'safe'],
    ...
    public function save($runValidaton = true, $attributes = null)
    {
        ...
        if (($this->getOldAttribute('status') != self::STATUS_SUSPENDED && $this->getAttribute('status') == self::STATUS_SUSPENDED) ||
            ($this->getOldAttribute('status') != self::STATUS_INACTIVE && $this->getAttribute('status') == self::STATUS_INACTIVE)) {
